...
class Generator extends \yii\gii\Generator
{
     ...
     * @var string
     */
    public $baseControllerClass = 'helpers\Controller';
    /**
     * @var string
     */
    ...
    {
        $columns = [];
        $likeColumns = []; // Only text/string columns for LIKE search
        ...
        // Collect only string/text columns for global search
        foreach ($columns as $column => $type) {
            // Include string types and exclude numeric/date types
            switch ($type) {
                case Schema::TYPE_TINYINT:
                case Schema::TYPE_SMALLINT:
                case Schema::TYPE_INTEGER:
                ...
                case Schema::TYPE_DATETIME:
                case Schema::TYPE_TIMESTAMP:
                    // Skip these — not suitable for text search
                    break;
                default:
                    // All others (string, text, char, varchar, etc.) → use LIKE
                    $likeColumns[] = $column;
                    break;
            }
        }
        // If no text columns, return empty
        if (empty($likeColumns)) {
            return ["// No text columns found for global search"];
        }
        // Generate the clean single-line condition
        $columnsList = "['" . implode("', '", $likeColumns) . "']";

        $condition = <<<PHP
                        \$query->andFilterWhere(
                                        self::ciLikeAny({$columnsList}, \$this->q)
                                    );
                        PHP;
        return [$condition];
    }
        ...
        foreach ($columns as $column => $type) {
            switch ($type) {
                ...
                default:
                    ...
                    $likeConditions[] = "->andFilterWhere(self::ciLike('{$column}', \$this->{$column}))";
                    break;
            } 
        }
