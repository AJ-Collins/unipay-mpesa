...
class AuthManager extends BaseManager
{
    public $protectedRoles = [
        'group'=>['sa' => 'System Administrators','user' => 'General Users'],
        'role'=>['su' => 'Super User', 'editor' => 'Editor', 'creator' => 'Creator', 'viewer' => 'Viewer', 'api' => 'API User', 'deletor' => 'Deletor', 'restore' => 'Restore']
    ];
    ...
    public $groupsCacheSuffix = 'groups';
    ...
    public function getGroupsByUser($userId)
    {
        ...
        $query = (new Query())->select('b.*')
            ...
            ->andWhere(['b.type' => Item::TYPE_GROUP]);

        $groups = $this->getDefaultGroupInstances();
        foreach ($query->all($this->db) as $row) {
            $groups[$row['name']] = $this->populateItem($row);
        }

        if ($this->cache !== null) {
            $this->cacheUserGroupsData($userId, $groups);
        }
    ...
    private function getUserGroupsCacheKey($userId)
    {
        return $this->cacheKey . $this->groupsCacheSuffix . $userId;
    }
    ...
    private function getUserGroupsCachedSetKey()
    {
        return $this->cacheKey . $this->rolesCacheSuffix;
    }
    ...
    private function cacheUserGroupsData($userId, $roles)
    {
        $cachedUserIds = $this->cache->get($this->getUserGroupsCachedSetKey());
        ...
        $this->cache->set($this->getUserGroupsCacheKey($userId), $roles);
        $this->cache->set($this->getUserGroupsCachedSetKey(), $cachedUserIds);
    }
}
