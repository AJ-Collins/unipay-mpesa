...
/*@method \yii\rbac\Item createGroup(string $name) Creates a new group with the specified name.
    */
     ...
     *
     * @return array An array of group items.
     */
    public function getGroups()
    {
        return $this->getItems(Item::TYPE_GROUP);
    }
    /**
     * Returns the named group.
     *
     * @param string $name The group name.
     * @return Group|null The group corresponding to the specified name. Null is returned if no such group.
     */
    public function getGroup($name)
    {
        $item = $this->getItem($name);
        return $item instanceof Item && $item->type == Item::TYPE_GROUP ? $item : null;
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        $permission = new class extends Item
        {
    ...
    public function getDefaultGroupInstances()
    {
        $result = [];
        foreach ($this->defaultGroups as $groupName) {
            $result[$groupName] = $this->createGroup($groupName);
        }
