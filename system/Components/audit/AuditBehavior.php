...
use helpers\migrations\AuditTableMigration;
...
class AuditBehavior extends Behavior
{
    ...
    {
        // Ignore if running from console (e.g., migrations, cron jobs)
        if (Yii::$app instanceof \yii\console\Application) {
            return [];
        }

        // Ignore specific model(s)
        $ownerClass = get_class($this->owner);
        $ignoredModels = [
            \helpers\log\AccessLogModel::class,
            \helpers\auth\LoginAttempt::class,

        ];

        if (in_array($ownerClass, $ignoredModels, true)) {
            return [];
        }
        // Default behavior if not ignored
        return [
        ...
        if (!$this->owner->isNewRecord) {
            ...
            foreach ($oldAttributes as $name => $oldValue) {
                ...
                if ($oldValue != $newValue && $name != 'updated_at') {
                    $log = new AuditModel();
                    $log->old_value =  (is_array($oldValue)) ? json_encode($oldValue, JSON_UNESCAPED_SLASHES) : $oldValue;
                    $log->new_value = (is_array($newValue)) ? json_encode($newValue, JSON_UNESCAPED_SLASHES) : $newValue;
                    $log->operation = $name == 'is_deleted' ? 'DELETE' : 'UPDATE';
                    $log->model_name = Yii::$app->controller->module->id . ': ' . $this->owner->formName();
                    $log->field_name = $name;
                    ...
                    $log->memory_max = memory_get_peak_usage(true);
            ...
            foreach ($newAttributes as $name => $value) {
                if ($name != 'created_at' || $name != 'updated_at') {
                    $log = new AuditModel();
                    $log->old_value = 'NA';
                    $log->new_value = (is_array($value)) ? json_encode($value, JSON_UNESCAPED_SLASHES) : $value;
